<?php


interface iPHRASES{
  public function get_phrase($type = "");
  public function insert_phrase($type = "", $phrase = "");
  public function read_from_file($file_path = "");
}

class PHRASES implements iPHRASES
{
  protected $phrases;
  protected $n_phrases;

  public function get_phrase($type = "")
  {
    if( !isset($this->phrases["$type"]) ) return -1;
    $pos = rand(0,$this->n_phrases - 1);
    return $this->phrases[$type][$pos];
  }

  public function insert_phrase($type = "", $phrase = "")
  {
    $this->phrases[$type][] = $phrase;
    if( isset($this->n_phrases["$type"]) )
      $this->n_phrases["$type"]++;
    else
      $this->n_phrases["$type"]=1;
  }

  public function read_from_file($file_path = "")
  {

  }
  
}

/*

$phrases["saluti"] = ["ciao","hola", "ciao, come va?", "hey, chi si rivede", "weee"];

$phrases["insulti"] = ["sei un coglione","sei un puppo", "si n'arancinu chi pedi", "ma d'unni niscisti?",
", sei così spaventoso che quando caghi la tua stessa merda dice che fai cagare!",
", sei come la minchia: sempre tra le palle",
", sei cosi scemo che guardi pure peppa pig...",
", tua mamma è come gli orsi: sempre in cerca di pesce",
", tua madre è come Buffon, ha sempre palle tra le mani",
" si bbabbiddiu",
" si tuttu nnacchiu",
" si bbabbu, loccu e cretinu"
];

$phrases["che fai"] = ["Eseguo i vostri ordini :(" , "E che dovrei fare, me ne sto qui solo soletto a girare nella ram",
"Niente, faccio quello per cui sono stato programmato",
"Stavo facendo una partita a prato fiorito, mi hai interrotto!",
"Secondo te? Attendo che qualcuno mi dia qualcosa da fare"];

$phrases["padre"] = [
"Gabriele, ovviamente...",
"Mio padre è Gabriele, ecco cosa succede quando un programmatore ha del tempo libero...",
"Sono il figlio di Atom, ma realmente mi ha scritto Gabriele",
"Eh, sapessi...\nMio padre era un commodore 64, purtroppo morì negli anni 90 ed io restai segregato in un floppy per ben ".
"26 anni. Nel 2016 qualcuno mi mise in un computer e così tornai finalmente libero. \nMa chi sto prendendo in giro, nel 2016".
"nessuno inserirebbe un floppy, e poi negli anni 90 manco esisteva telegram... Mio padre è Gabriele, e non sa nemmeno lui perchè mi ha scritto."];

$phrases["canzone"][0] =
"Li furisteri ca 'nSicilia sunnu
la guardanu ccu granni meraviglia,
dicinu ca nun c'è na tuttu u munnu
'nisula ca la nostra s'assumiglia.

La conca d'oru è chidda ca straluci
supra la nivi mungibbeddu ridi,
spanni lu faru la so janca luci
l'Anapu scurri 'mezzu a li papiri.

Sicilia Sicilia canta la pasturedda
Sicilia Sicilia jocca la funtanedda
l'aria e lu suli inchinu l'arma di puisia,
Sicilia Sicilia tu si la terra mia.

L'aceddi pipitianu a matinata
tra zagari d'aranci e minnuliddi,
cu la vuccuzza duci e 'nzuccarata,
cantanu comu parlanu li ziti.

Lu suli spunta e tutta la campagna
d'oru zecchinu pari arracamata
li picureddi supra la muntagna
rusicanu l'irbuzza 'mbarsamata.

Sicilia Sicilia canta la pasturedda
Sicilia Sicilia jocca la funtanedda
l'aria e lu suli inchinu l'arma di puisia,
Sicilia Sicilia tu si la terra mia";

$phrases["barzelletta"] = [
"Cos'è che ti sta sul cazzo?...la figa! ",

"In che modo canta una prostituta? A cappella ",

"Come si pesca con un computer? si usa la rete! ",

"Etrnità: tempo che trascorre dal \"doppio click\" all'apertura del documento cliccato.
Hadware: parte del computer che riceve i colpi quando si blocca il computer.
Nanosecondo: tempo che impiega un computer per distruggere il lavoro di un'intera giornata.
Progammatore: chi risolve in modo incomprensibile ma costosissimo un problema che non sapevi di avere. ",

"Un informatico si presenta sul posto di lavoro. E` il suo primo giorno...
Il dirigente gli mette una scopa in mano e gli dice:
- Ecco, questa è una scopa, come prima cosa potresti dare una spazzata all`ufficio...
Il giovane replica:
- Una scopa?!? Ma guardi che io sono un informatico, mi considero un genio!!!
Ed il capo:
- Hai ragione, scusa, vieni di là che ti faccio vedere come funziona...",

"Ragazza stufa scappa di casa… I genitori muoiono di freddo.",

"Nuova lavatrice lanciata sul mercato: 5 feriti e 2 morti!!!",

"E come disse il cestino…. \"io mi rifiuto!!\" "
];

$phrases["wee"] = ["wee", "wereweee", "i leliii", "weeeeeeeeeeeeeeeeeeeeee"];


$n["saluti"] = count($phrases["saluti"]) - 1;
$n["insulti"] = count($phrases["insulti"]) - 1;
$n["che fai"] = count($phrases["che fai"]) - 1;
$n["padre"] = count($phrases["padre"]) - 1;
$n["canzone"] = count($phrases["canzone"]) - 1;
$n["barzelletta"] = count($phrases["barzelletta"]) - 1;
$n["wee"] = count($phrases["wee"]) - 1;
*/


?>
